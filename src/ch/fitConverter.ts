...
export class WorkoutConverter {
  ...
  private convertWorkoutToSteps(
    ...
    if (parsedWorkout.isStructured) {
      ...
      steps.push(
        ...this.createStructuredWorkoutSteps(parsedWorkout, workout, paceZones)
      );
    } else {
  ...
  private convertDayDetailsToSteps(
    ...
    if (parsedWorkout.isStructured) {
      ...
      steps.push(
        ...this.createStructuredWorkoutStepsFromDayDetails(
          parsedWorkout,
          dayDetails,
          paceZones
        )
      ...
      // Simple workout
      steps.push(
        this.createSimpleWorkoutStepFromDayDetails(dayDetails, 0, paceZones)
      );
    }
  ...
  private parseWorkoutDescription(description: string): ParsedWorkout {
    ...
    if (
      lowerDesc.includes("tempo") ||
      lowerDesc.includes("threshold") ||
      lowerDesc.includes("lt")
    ) {
      return {
        isStructured: true,
        type: "tempo",
  ...
  private createStructuredWorkoutSteps(
    ...
    switch (parsed.type) {
        ...
        for (let i = 0; i < parsed.repetitions!; i++) {
          ...
          steps.push({
            ...
            ...(parsed.workUnit === "min"
              ...
              : {
                  durationDistance: this.convertDistanceToMeters(
                    parsed.workDuration!,
                    parsed.workUnit!
                  ),
                }),
          ...
          if (i < parsed.repetitions! - 1) {
            steps.push({
              ...
              durationType: FitDurationType.Time,
              durationTime:
                parsed.restDuration! * (parsed.restUnit === "min" ? 60 : 1),
              targetType: FitTargetType.HeartRate,
              targetHrZone: 1,
            });
          }
      ...
      case "warmup_main_cooldown":
        ...
        const totalDistance = this.convertDistanceToMeters(
          workout.distance,
          workout.units
        );
        ...
        steps.push({
          ...
          intensity:
            parsed.type === "tempo" ? FitIntensity.Active : FitIntensity.Active,
          durationType: FitDurationType.Distance,
  ...
  private createSimpleWorkoutStep(
    ...
    const intensity =
      this.getIntensityFromTags(workout.tags) ||
      this.getIntensityFromPace(workout.pace);
    const distanceInMeters = this.convertDistanceToMeters(
      workout.distance,
      workout.units
    );
    ...
    return {
      ...
      intensity,
      durationType:
        distanceInMeters > 0 ? FitDurationType.Distance : FitDurationType.Open,
  ...
  private createSimpleWorkoutStepFromDayDetails(
    ...
    const intensity =
      this.getIntensityFromTags(dayDetails.tags) ||
      this.getIntensityFromPace(dayDetails.pace);
    const distanceInMeters = this.convertDistanceToMeters(
      dayDetails.dist,
  ...
  private getIntensityFromPace(pace?: string): FitIntensity {
    ...
    if (lowerPace.includes("threshold") || lowerPace.includes("tempo"))
      return FitIntensity.Active;
    if (lowerPace.includes("interval") || lowerPace.includes("repetition"))
  ...
  private parseIntensityFromText(text: string): FitIntensity {
    ...
    if (
      ...
      lowerText.includes("lt")
    ) {
      return FitIntensity.Active;
    }
    if (
      lowerText.includes("interval") ||
      lowerText.includes("vo2") ||
  ...
  private getTargetValuesForPace(
    pace?: string,
    _paceZones?: PaceZones
  ): Partial<FitWorkoutStep> {
    if (!pace) {
      return {
    ...
    if (lowerPace.includes("recovery")) {
      ...
      lowerPace.includes("interval") ||
      lowerPace.includes("repetition")
    ) {
