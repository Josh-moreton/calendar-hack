// Simple test to verify fitConverter functionality
describe('FIT Converter Basic Test', () => {
  it('should pass basic test without imports', () => {
    expect(1 + 1).toBe(2);
  });
  
  it('should test dynamic import', async () => {
    try {
      const { WorkoutConverter } = await import('./fitConverter');
      expect(WorkoutConverter).toBeDefined();
