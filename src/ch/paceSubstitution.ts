...
import { calculateTrainingPaces, formatPace, RaceTime, RaceDistance, RACE_DISTANCES } from './paceCalculator';
 ...
 */
export function substitutePaces(text: string, paceSettings: PaceSettings | null): string {
  if (!paceSettings) {
    return text;
  }
  ...
  try {
    ...
    if (paces.marathon) {
      ...
      result = result.replace(PACE_PATTERNS.marathon, formatPace(paces.marathon));
    }
    
    if (paces.threshold) {
      result = result.replace(PACE_PATTERNS.tempo, formatPace(paces.threshold));
      result = result.replace(PACE_PATTERNS.threshold, formatPace(paces.threshold));
    }
    
    if (paces.interval) {
      result = result.replace(PACE_PATTERNS.interval, formatPace(paces.interval));
    ...
    if (paces.repetition) {
      result = result.replace(PACE_PATTERNS.repetition, formatPace(paces.repetition));
      result = result.replace(PACE_PATTERNS.repetitions, formatPace(paces.repetition));
    }
