...
export function getPaceByZone(paces: PaceZones, zone: string): number | null {
  ...
  if (
    normalizedZone.includes("threshold") ||
    normalizedZone.includes("lt") ||
    normalizedZone.includes("tempo")
  ) {
  ...
  if (
    normalizedZone.includes("interval") ||
    normalizedZone.includes("vo2") ||
    normalizedZone.includes("5k")
  ) {
    return paces.interval;
  }
  if (
    normalizedZone.includes("repetition") ||
    normalizedZone.includes("rep") ||
    normalizedZone.includes("mile") ||
    normalizedZone.includes("800")
  ) {
...
export function predictRaceTime(
  ...
  if (distanceInMeters <= 1609)
    adjustmentFactor = 1.12; // Mile and shorter
  else if (distanceInMeters <= 5000)
    adjustmentFactor = 1.02; // 5K
  else if (distanceInMeters <= 10000)
    adjustmentFactor = 0.98; // 10K
  else if (distanceInMeters <= 21097)
    adjustmentFactor = 0.92; // Half marathon
