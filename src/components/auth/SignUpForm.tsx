...
export const SignUpForm: React.FC<SignUpFormProps> = ({ onSuccess, redirectTo = '/' }) => {
  const { signUp, signInWithGoogle, signInWithApple, signInWithStrava, loading, error } = useAuth();
  ...
  const handleSocialSignIn = async (provider: 'google' | 'apple' | 'strava') => {
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="mt-8 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <button
              type="button"
              onClick={() => handleSocialSignIn('strava')}
              disabled={loading || socialLoading !== null}
              className="group relative w-full flex justify-center py-2 px-4 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {socialLoading === 'strava' ? (
                <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-600"></div>
              ) : (
                <>
