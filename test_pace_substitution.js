...
try {
  ...
  console.log(`Total workouts with descriptions: ${workoutsWithDescriptions.length}`);
  ...
  if (pacePatterns.length === 0) {
    console.log("No '@...pace' patterns found. Let's look for other pace-related text:");
    
    workoutsWithDescriptions.forEach((workout, index) => {
      const desc = workout.description;
      if (desc.toLowerCase().includes('race pace') || desc.toLowerCase().includes('@ ')) {
        console.log(`Workout ${index + 1}: "${workout.title}"`);
