...
const PACE_PATTERN_MAPPINGS = {
  ...
  '@ marathon race pace': { zone: 'marathon', description: 'marathon race pace' },
  '@ 15K to half marathon race pace': { zone: 'threshold', description: 'lactate threshold effort' },
  '@ 15K to half marathon pace': { zone: 'threshold', description: 'lactate threshold effort' },
  '@ 800 m to mile race pace': { zone: 'repetition', description: 'neuromuscular power' },
...
function convertPlan(planPath) {
  ...
  const convertedPlan = {
    ...
    schedule: planData.schedule.map(week => ({
      ...week,
      workouts: week.workouts.map(workout => convertWorkoutToPaceStructure(workout))
    }))
  };
  ...
  console.log(`  Conversion rate: ${((convertedWorkouts / totalWorkouts) * 100).toFixed(1)}%`);
